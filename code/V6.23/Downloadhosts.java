...
public class Downloadhosts {
    ...
    public static void Download(final ProgressDialog mpDialog, final Context context, final String hosts_url, Button btn_AD, Button btn_RE, Button btn_AR, final int umeng, final int mybus, final int jiajia, final int tieba, final int nj) {
        btn_AD1 = btn_AD;
        btn_RE1 = btn_RE;
        btn_AR1 = btn_AR;
        context1 = context;
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    ...
                    if (nj == 1) {
                        String temp = (context.getString(R.string.URL_nj));
                        FileOutputStream fos = new FileOutputStream(DATABASE_PATH + "/hosts", true);//true表示在文件末尾追加
                        fos.write(temp.getBytes());
                        fos.close();//流要及时关闭
